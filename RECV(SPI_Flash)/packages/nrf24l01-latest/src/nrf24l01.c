...
int nrf24_init(nrf24_cfg_t *pt)
{
    ...
    set_address_width5();
    set_tx_rp0_address5(pt->address);	/* 默认只配了数据通道0的地址 */
    nrf24l01_multichannel();	/* 添加的 API */
...
uint8_t nrf24_prx_run(void){
	...
	if(pipe_state & 0x40){
		...
		hal_nrf24l01_port.reset_ce();
		if((pipe_state & 0x0E) == 0x00){
			
			rlen = get_top_rxfifo_width();
			read_rxpayload(RxBuf_P0, rlen);
		
		}else if((pipe_state & 0x0E) == 0x02){
		
			rlen = get_top_rxfifo_width();
			read_rxpayload(RxBuf_P1, rlen);
		
		}else if((pipe_state & 0x0E) == 0x04){
		
			rlen = get_top_rxfifo_width();
			read_rxpayload(RxBuf_P1, rlen);
		
		}else if((pipe_state & 0x0E) == 0x06){
		
			rlen = get_top_rxfifo_width();
			read_rxpayload(RxBuf_P1, rlen);
		
		}else if((pipe_state & 0x0E) == 0x08){
		
			rlen = get_top_rxfifo_width();
			read_rxpayload(RxBuf_P1, rlen);
			
		}else if((pipe_state & 0x0E) == 0x0A){
